...
using Godot;
using static Jomolith.Player.Humanoid.State.PlayerLogic;
...
{
    [Meta]
    public abstract partial record RunningBase : Alive, IGet<Inputs.Jump>, IGet<Inputs.OnFloor>
    {
        ...
        public virtual Type On(in Inputs.Jump input)
        {
            return To<Jumping>();
        }

        public virtual Type On(in Inputs.OnFloor input)
        {
            ...
            if (desiredYVelocity != null && desiredYVelocity > 0)
            {
                ...
                Output(new Outputs.ApplyForce(antiGravityForce + groundForce, Vector3.Zero));
            }
